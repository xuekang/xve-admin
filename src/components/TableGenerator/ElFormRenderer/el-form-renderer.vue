...
<script>
// import { _set, this._.isEqual, this._.cloneDeep } from 'lodash'
...
export default {
  ...
  watch: {
    ...
    value: {
      handler(v, oldV) {
        if (!v || this._.isEqual(v, oldV)) return
  ...
  mounted() {
     ...
     */
    this.initValue = this._.cloneDeep(this.value)
    this.$nextTick(() => {
      // proxy
  ...
  methods: {
    ...
    resetFields() {
       ...
       *   4. 因为 this._.isEqual(v, oldV)，所以没有触发 v-model 更新
       */
      this.value = this._.cloneDeep(this.initValue)
    ...
    setValueFromModel() {
      ...
      if (!this._.isEqual(this.value, newValue)) this.value = newValue
    },
    /**
     * 更新表单数据
    ...
    setOptions(id, options) {
      this._.set(this.options, id, options)
